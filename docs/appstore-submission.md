...
```
운동과 휴식을 반복하는 인터벌 트레이닝 타이머. 애플워치에서 바로 실행하고, 잠금화면과 다이나믹 아일랜드로 남은 시간을 확인하세요. 모든 데이터는 기기에만 저장됩니다.
```
...
타바타, HIIT, 서킷 트레이닝, 플랭크처럼 시간을 나눠 하는 모든 운동에 맞게 자유롭게 설정할 수 있습니다.

■ 주요 기능
• 운동 시간, 휴식 시간, 세트 수를 자유롭게 설정
• 단계마다 이름과 시간을 정하는 상세 타이머 (예: 버피 40초 → 휴식 20초)
• 구간이 끝나기 3초 전 카운트다운 비프음과 진동
• 자동 진행 / 세트마다 멈춤 / 수동 등 진행 방식 선택
• 여러 타이머를 만들어 두고 넘겨가며 사용

■ 잠금화면에서도 이어서
• 잠금화면 실시간 현황(Live Activity)과 다이나믹 아일랜드로 남은 시간 확인
• 홈 화면 위젯
...
```
인터벌,타바타,HIIT,운동타이머,홈트,스톱워치,세트,휴식,크로스핏,플랭크,루틴,인터벌타이머,워치
```
...
`web/` 폴더에 랜딩 + `/support` + `/privacy-policy` 정적 사이트가 있다.
[app.netlify.com](https://app.netlify.com) → Deploy manually → `web` 폴더 드래그로 배포.
자세한 절차는 `web/README.md` 참고.
